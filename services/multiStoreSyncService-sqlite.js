...
class MultiStoreSyncService {
  constructor() {
    ...
    this.dbToUse = sqliteDb; // Siempre usar SQLite

    console.log('🔍 Configuración de sincronización SQLite:');
    console.log('- Base de datos: SQLite (simplificado)');
    console.log('- Tiendas:', this.stores.length);
  }

  async syncAllStores(fromDate, toDate) {
    console.log(`🔄 Iniciando sincronización SQLite desde ${fromDate} hasta ${toDate}`);
  ...
  async syncStore(storeConfig, fromDate, toDate) {
    ...
    try {
      ...
      for (const order of orders) {
        try {
          // Insertar orden
          console.log(`  📝 Insertando orden: ${order.idSaleOrder}`);
          console.log(`  📊 Datos de orden:`, {
            orderId: order.idSaleOrder,
          ...
          const orderResult = await this.dbToUse.prepare(`
          ...
          for (const product of orderProducts) {
            try {
              ...
              const productResult = await this.dbToUse.prepare(`
      ...
      console.log(`✅ ${storeConfig.store_name} completada: ${recordsProcessed} registros de ${orders.length} órdenes`);
